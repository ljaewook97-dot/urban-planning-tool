...
export default async function handler(req) {
  ...
  const systemInstruction = `당신은 대한민국 공공기관 발주처에 제출하는 도시계획 ${docType}를 작성하는 전문 도시계획 엔지니어입니다.
다음 원칙을 반드시 따르세요:
...
공공기관 발주처를 납득시킬 수 있는 전문적 ${docType}를 법적 근거, 타당성 분석, 검토 의견, 결론 순으로 완성해 주세요.`;
  ...
  try {
    ...
    if (!upstream.ok) {
      ...
      const msg = err?.error?.message || `Gemini API 오류 (${upstream.status})`;
      return new Response(JSON.stringify({ error: msg }), { status: 502 });
    }

    // Gemini SSE → 클라이언트로 그대로 전달
    // 클라이언트에서 Gemini SSE 포맷 파싱
